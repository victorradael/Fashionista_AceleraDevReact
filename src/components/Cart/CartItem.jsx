...
export default function CartItem({ product }) {
  ...
  function filterSize(sizes) {
    const filteredSizes = sizes.filter(
      (size) => size.sku === product.selectedSize
    );
  ...
  return (
    <li className="cart-item">
      ...
      <div className="cart-item__infos">
        ...
        <label className="cart-item__text">
          Tamanho: {filterSize(product.sizes)}
        </label>
        <label className="cart-item__text">
          Unidade: {product.actual_price}
        </label>

        <div className="cart-item__quantity">
          <button
            className="cart-item__btn-increment"
            onClick={() => increment(product.selectedSize)}
          >
            +
          </button>
          <label className="cart-item__text">{product.quantity}</label>
          <button
            className="cart-item__btn-decrement"
            onClick={() => decrement(product.selectedSize)}
          >
            -
          </button>
        </div>
      ...
      >
        <AiOutlineCloseCircle size={24} color="orangered" />
      </button>
    </li>
  );
}
