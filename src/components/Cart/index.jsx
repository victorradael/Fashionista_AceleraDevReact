...
export default function Cart() {
  ...
  function handleCheck() {
    if (cartCounter > 0) {
      ...
      dispatch(actionsToast.addToast("Compra finalizada com sucesso!", false));
    } else
      dispatch(actionsToast.addToast("Sacola vazia, selecione um item", true));
  }

  return (
    <>
      <button className="cart__btn-icon" onClick={openCart}>
        <BsBagFill size={24} />
        <sup className="cart__counter">{<span>{cartCounter}</span>}</sup>
      </button>
